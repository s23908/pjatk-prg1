...
auto evaluate_multiplication(std::stack<double> &stack) -> void {
  if (stack.size() < 2) {
    throw std::logic_error{"not enough opreands for *"};
  }
  auto const b = pop_top(stack);
  auto const a = pop_top(stack);
  stack.push(a * b);
...
auto evaluate_inchtocm(std::stack<double> &stack) -> void {
  if (stack.size() < 1) {
    throw std::logic_error{
        "not enough operands for conversion from inch to cm"};
  }
  auto const a = pop_top(stack);
  double i = 2.54;
  stack.push(a / i);
